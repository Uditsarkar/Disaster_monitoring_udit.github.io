export const StatsCtrl = {
    ...
    onViewReady: function() {
        ...
        webix.ajax().get("data.json", function(text) {
            ...
            StatsCtrl.loadRainfallData(); // Load rainfall data after tweets data
        });
    },

    loadRainfallData: function() {
        webix.ajax().get("Z:/Twitter/webix/rainfall.csv", function(text) {
            StatsCtrl.rainfallData = StatsCtrl.parseCSV(text);
    ...
    initializeCharts: function() {
        ...
        const rainfallData = StatsCtrl.rainfallData.reduce((acc, day) => {
            const date = day['Date'];
            acc[date] = parseFloat(day['Rainfall (mm)']);
            return acc;
        }, {});

        // Combine tweets and rainfall data
        const combinedData = Object.keys(tweetsPerDay).reduce((acc, date) => {
            acc[date] = {
                tweets: tweetsPerDay[date],
                rainfall: rainfallData[date] || 0
            };
            return acc;
        }, {});
        ...
        const sortedDates = Object.keys(combinedData)
            ...
            .sort((a, b) => a - b)
            .filter(date => date <= new Date("2023-08-24"))  // Adjust the date as necessary
            .map(date => date.toISOString().split('T')[0]);

        // Prepare bar chart data with sorted and filtered dates
        const barChartData = {
            labels: sortedDates,
            datasets: [{
                label: 'Tweets per Day',
                data: sortedDates.map(date => combinedData[date].tweets || 0),
                backgroundColor: '#36A2EB',
                yAxisID: 'y-tweets'
            }, {
                label: 'Rainfall (mm)',
                data: sortedDates.map(date => combinedData[date].rainfall || 0),
